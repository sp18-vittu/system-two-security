...
function InsightsPagesRule({ tablelist, paramsdata, setdeletvale, loader }: any) {
  const { id } = useParams()
  ...
  const handleOpenCollection = (data: any) => {
    if (data == 'ctireport') {
      ...
      data == 'singlecollecionmove' ||
      data == 'cticollecioncopy' ||
      data == 'Threatbreif'
    ) {
      setAnchorE6(null)
      setCollectionorcti(data)
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params: any) => {
        return (
          <>
            {params?.row?.modifiedTime ? (
              <div>
                <TimeAgo createdAt={params.row.modifiedTime} />
              </div>
            ) : (
              <div>-</div>
            )}
                  ...
                  <ul>
                    ...
                    <>
                      <button className='flex'>
                        <li
                          className='text-sm font-medium m-1'
                          onClick={() => handleOpenCollection('Threatbreif')}
                        >
                          Copy to Collection
                        </li>
                      </button>
  ...
  const filterdata: any = tablelist
    .filter(
      (collection: any) =>
        ...
        collection.ctiName.toLowerCase().includes(search.toLowerCase()),
    )

  return (
    <div className='w-full mt-[-30px]'>
      {paramsdata == 'ctisigma' && (
        <div className='flex items-center justify-between gap-4 p-4 mt-12 w-full max-lg:flex-wrap'>
          {/* Search Box */}
          ...
          <div className='flex items-center gap-4 ml-auto space-x-3'>
            ...
            <button
              ...
              onClick={() => handleOpenCollection('ThreatbreifAll')}
            >
              Copy to Collection
            </button>
...
export default InsightsPagesRule
