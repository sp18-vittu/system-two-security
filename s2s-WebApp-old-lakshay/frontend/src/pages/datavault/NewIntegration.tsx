...
import { FEEDLY_DELETE_RESET, deleteFeedly, feedlyGet } from '../../redux/nodes/feedlyform/action'
import { SPLUNK_DELETE_RESET } from '../../redux/nodes/splunkform/action'

function NewIntegration() {
  const navigateTo = useNavigate()
  const dispatch = useDispatch()
  const connectformfeedly = () => {
    navigateTo(`/app/feedlyform`)
  }
  ...
  const [feedDatas, setfeedData] = useState(null as any)
  console.log('feedDatas', feedDatas)

  const [IntegrationCards, setIntegrationCards] = useState([] as any)
  ...
  const feedgetmethod = () => {
    dispatch(feedlyGet() as any).then((respons: any) => {
      ...
      setIntegrationCards(respons.payload)
  ...
  const localStore = localStorage.getItem('getfeeddata')
  const localdata = JSON.parse(localStore as any)

  const handleUpdateClick = (feedData: any) => {
    navigateTo(`/app/feedlyform`, { state: [{ feed: feedData }] })
  }

  const ondisconnect = (data: any) => {
    dispatch(deleteFeedly(feedDatas?.id) as any).then((response: any) => {
      if (response) {
        dispatch(feedlyGet() as any).then((respons: any) => {
          localStorage.removeItem('getfeeddata')
          setfeedData(null)
        })
      }
    })
  ...
  return (
    <div className='p-2 h-[88vh]'>
      <div className='bg-[#0C111D]  lg:mr-5 lg:grid lg:grid-cols-3 lg:gap-4 lg:mt-3 lg:ml-3.5 lg:mr-3.5 lg:pt-3 grid grid-cols-1 gap-4 mt-3 ml-3.5 mr-3.5 md:pt-0 pt-32 text-white'>
        <div>
          {IntegrationCards.map((response: any, index: any) => {
            const status =
              response.sourceName == 'Feedly'
                ? 'Feedly'
                : response.sourceName == 'Elastic'
                ? 'Elastic'
                : 'Splunk'
            return (
              <>
                {response.integrationSourceType === 'SIEM' && (
                  <>
                    <div className='bg-[#344054] mt-2 rounded-lg text-white p-5' key={index}>
                      <div className='flex justify-between mt-2'>
                        <div>
                          <p className='bg-[#98A2B3] rounded-xl ps-1.5 text-[12px] w-10'>
                            {response.integrationSourceType}
                          </p>
                        </div>
                        {response?.connected && (
                          <div>
                      ...
                      <div className='pt-2 flex justify-between'>
                        <div>{svgIcons[status]}</div>
                        {response?.connected && (
                          <div>
                            <p
                              className='text-[#7F56D9] mt-5 cursor-pointer'
                              onClick={(e) => ondisconnect(response.id)}
                            >
                              Disconnect
                            </p>
                          </div>
                      ...
                      {!response?.connected && (
                        <div className={`flex justify-between mt-1 `}>
                          <div className={``}>
                            <p
                              className={`text-gray-300 mt-2 font-inter text-lg font-medium leading-7`}
                            >
                              {response.sourceName}
                            </p>
                          </div>
                      ...
                      {response?.connected && (
                        <div
                          className={`flex justify-between mt-1 ${
                            response.sourceName === ' Elastic' && 'mt-2 border border-red'
                          }`}
                        >
                          <div>
                          ...
                          <div
                            ...
                            onClick={() => handleUpdateClick(feedDatas)}
                          >
                            <div>
                              <p className='text-gray-300 font-inter text-lg font-medium leading-7'>
                                Configure
        ...
        <div>
          {IntegrationCards.map((response: any, index: any) => {
            ...
            return (
              <>
                {response.integrationSourceType === 'CTI' && (
                  <>
                    <div className='bg-[#344054] mt-2 rounded-lg text-white p-5' key={index}>
                      <div className='flex justify-between mt-2'>
                        <div>
                          <p className='bg-[#98A2B3] rounded-xl w-8 ps-1.5 text-[12px]'>
                            {response.integrationSourceType}
                          </p>
                        </div>
                      ...
                      {(feedDatas ? feedDatas?.connected : localdata?.connected) && (
                        <div className='flex justify-between mt-1 '>
                          <div>
                            <p className='text-gray-300 mt-2 font-inter text-lg font-medium leading-7'>
                              {response.sourceName}
