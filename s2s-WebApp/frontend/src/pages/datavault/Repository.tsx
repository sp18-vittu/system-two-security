import { Box, Button, MenuItem, tooltipClasses } from '@mui/material'
...
import { vaultPermissionList } from '../../redux/nodes/vaultPermission/action'
import local from '../../utils/local'
import { ctiReportFileList, updateFileTitle } from '../../redux/nodes/cti-report/action'
...
import { inspectUrlSigmaId } from '../../redux/nodes/Inspects/action'
...
const Repository = (props: any) => {
  ...
  const { setData, setDetail, setInsightDetail, setCopyFiles, wssProvider, setWssProvider }: any =
    useData()
  ...
  const [viewAccesstype, setViewAccesstype] = useState('')
  ...
  useEffect(() => {
    dispatch(vaultPermissionList(id) as any)
  ...
  const { loading, success: savesuccess, dataVaultDelete } = savedocumemt
  ...
  const [inspectPage, setInspectPage] = useState('Repository' as any)

  const onInspectView = async (e: any, params: any) => {
    dispatch(inspectUrlSigmaId(viewprams?.row?.url) as any)
      .then((res: any) => {
        if (res.type == 'INSPECT_CTI_URL_SUCCESS') {
          if (res?.payload?.length > 0) {
            setAnchorE6(null)
            setCtiSectionId(res?.payload)
            setInspectView(true)
            setInspectPage('Repository')
          }
        }
      })
      .catch((err: any) => {})
  ...
  const onViewimage = async (e: any, params: any) => {
    setAnchorE6(null)
    if (viewprams?.row?.reportSource == 'PDF') {
      try {
        const { data } = await Axios.get(
          `${environment.baseUrl}/data/view-cti-report-pdf?datavaultId=${viewprams?.row?.datavault?.id}&ctiId=${viewprams?.row?.id}`,
          {
            responseType: 'blob',
            headers: {
  ...
  const handleSortingOrderChange = (event: any) => {
    const sortedValues = ctiDefaultFiles
      ?.slice()
      .sort((a: any, b: any) =>
    ...
    if (ctiReportFilesArr.length > 0) setctiReportFilesArr(sortedValues)
    else setCtiDefaultFiles(sortedValues)
  ...
  const onSearch = (e: any) => {
    ...
    let data: any = [...ctiReportFiles]
    data = data?.filter((value: any) =>
      value?.ctiName?.toLowerCase().includes(query?.toLowerCase()),
    )
    const filteredData = data.filter(
      (item: any) =>
        item?.status?.toLowerCase().includes(query?.toLowerCase()) ||
        item?.mitreLocation?.toLowerCase().includes(query?.toLowerCase()) ||
        item?.url?.toLowerCase().includes(query?.toLowerCase()) ||
        item?.ctiName?.toLowerCase() === query?.toLowerCase() ||
        item?.datavault?.name?.toLowerCase() === query?.toLowerCase() ||
        moment(item?.creationTime).format('DD-MMM HH:mm').toLowerCase() ===
          moment(query).format('DD-MMM HH:mm')?.toLowerCase() ||
        moment(item?.creationTime).format('DD MMM').toLowerCase() ===
          moment(query).format('DD MMM')?.toLowerCase() ||
        moment(item?.creationTime).format('HH:mm').toLowerCase() ===
          moment(query).format('HH:mm')?.toLowerCase() ||
        moment(item?.creationTime).format('HH').toLowerCase() ===
          moment(query).format('HH')?.toLowerCase(),
    )
    setctiReportFilesArr(filteredData)
    setCtiDefaultFiles(filteredData)
  }
  ...
  const handleClickdot = (event: any, params: any) => {
    setglobaldata(params.row.global)
    setveiwprams(params)
    ...
    setViewAccesstype(params.row.accessType)
...

    {
      ...
      renderCell: (params) => (
        <>
          {params.row.reportSource === 'WEB' ? (
            <>
              <div className='flex justify-center border-2 border-[#7A5AF8] p-1 px-2 rounded-2xl cursor-default'>
                <div className='text-[#7A5AF8]'>Community</div>
              </div>
            </>
          ) : (
            <>
              <div className='flex justify-center border-2 border-[#1570EF] p-1 px-2 rounded-2xl cursor-default'>
              ...
              <div className='right-0 px-[2px] py-[6px] text-[#344054] w-[200px]'>
                <ul>
                  ...
                  <li className='py-1'>
                    <button
                      className={`${params.row.disabled ? `flex cursor-not-allowed` : `flex`}`}
                      onClick={(e: any) => onInspectView(e, params?.row)}
                    >
                      <svg
                        viewBox='0 0 15 16'
                        fill='none'
                      ...
                      >
                        <path
                          d='M8.33341 7.33301H4.33341M5.66675 9.99967H4.33341M9.66675 4.66634H4.33341M12.3334 6.99967V4.53301C12.3334 3.4129 12.3334 2.85285 12.1154 2.42503C11.9237 2.0487 11.6177 1.74274 11.2414 1.55099C10.8136 1.33301 10.2535 1.33301 9.13341 1.33301H4.86675C3.74664 1.33301 3.18659 1.33301 2.75877 1.55099C2.38244 1.74274 2.07648 2.0487 1.88474 2.42503C1.66675 2.85285 1.66675 3.4129 1.66675 4.53301V11.4663C1.66675 12.5864 1.66675 13.1465 1.88474 13.5743C2.07648 13.9506 2.38244 14.2566 2.75877 14.4484C3.18659 14.6663 3.74664 14.6663 4.86675 14.6663H6.66675M13.6667 14.6663L12.6667 13.6663M13.3334 11.9997C13.3334 13.2883 12.2887 14.333 11.0001 14.333C9.71142 14.333 8.66675 13.2883 8.66675 11.9997C8.66675 10.711 9.71142 9.66634 11.0001 9.66634C12.2887 9.66634 13.3334 10.711 13.3334 11.9997Z'
                          stroke='#344054'
                          stroke-width='1.5'
                          stroke-linecap='round'
                          stroke-linejoin='round'
                      ...
                      <li className='mx-0.5 text-sm font-medium'>Inspect</li>
                    </button>
                  </li>
                  <li className='py-1'>
                  ...
                  <li className='py-1'>
                    {(getroleName?.roleName !== 'USER' &&
                      ...
                      viewAccesstype !== 'READ') ||
                    (getroleName?.roleName === 'USER' && viewAccesstype !== 'READ') ||
                    (getroleName?.roleName === 'DATAVAULT_ADMIN' && viewAccesstype !== 'READ') ? (
                      <button type='submit' onClick={() => {}} style={{ display: 'flex' }}>
                        <svg
                          className='mx-2 mt-0.5'
                          width='16'
                          height='16'
                        ...
                        <li className='mx-0.5 text-sm font-medium'>Delete</li>
                      </button>
                    ) : null}
                  </li>
                </ul>
  ...
  // ******************** api Integration*************************
  const datactiReport = useSelector((state: any) => state.datactiReportreducer)
  const { ctiReportFiles } = datactiReport
  ...
  const imagepart = {
    opacity: 0.5,
    PointerEvent: 'none' /* prevent interaction with the disabled row */,
  }
    ...
    <>
                ...
                >
                  ...
                  >
                    <li className='mb-4 md:mb-0 lg:mb-0 md:pr-2 lg:pl-2' data-te-nav-item-ref>
                      <a
                        className='sm:text-lgcursor-pointer text-white text-emerald-900 font-semibold lg:text-sm leading-5 underline underline-offset-8 transition duration-200 hover:text-white-900 hover:ease-in-out focus:text-emerald-900 disabled:text-black/30 motion-reduce:transition-none lg:px-2 [&.active]:text-emerald-900'
                    ...
                    {getroleName?.roleName !== 'USER' && (
                      <>
                        <li className='mb-4 lg:mb-0 lg:pr-2 md:mb-0 md:pr-2' data-te-nav-item-ref>
                          <a
                            className='sm:text-lg cursor-pointer text-[#667085] underline-offset-8 transition duration-200 lg:px-2 font-semibold lg:text-sm leading-5 blueGray-500 transition duration-200 lg:px-2'
                            onClick={vaultpermission}
                            data-te-nav-link-ref
                          >
                            Sigma Files
                          </a>
                  ...
                  <div className='mr-2 '>
                    <div className='relative inline-flex lg:top-0 lg:pt-0 '>
                      <div className='relative flex flex-wrap items-stretch '>
                        <input
                          type='search'
                          className='relative m-0 block w-70 h-9 flex-auto rounded-l-lg p-1.5 border-t border-l border-b border-solid border-neutral-300 bg-white text-base font-normal leading-[1.6] text-neutral-700'
                          placeholder='Search...'
                          aria-label='Search'
      ...
      >
        <DataGrid
          ...
          rows={ctiReportFilesArr?.length > 0 ? ctiReportFilesArr : ctiDefaultFiles}
          columns={columns}
          loading={reloading}
          classes={{
          ...
          onSelectionModelChange={(ids: any, newSelection: any) => {
            ...
            const selectedRowData: any = ctiReportFilesArr?.filter((ctireportFiles: any) =>
              ...
              <div className='relative my-6 w-2/6'>
                <div className='border-0 rounded-lg shadow-lg relative flex flex-col bg-white outline-none focus:outline-none'>
                  ...
                  <div className='relative p-5 flex-auto'>
                    ...
                    {err && err.urltitle && (
                      <>
                        <span className='text-red-500 text-[12px] px-1'>
                          {err?.urltitle?.message}
                        </span>
                      </>
                    )}
                  </div>
          ...
          <div className='backdrop-blur-sm justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none'>
            <div className='relative w-full h-full p-[32px] mx-auto'>
              <div className='p-[20px] border-0 rounded-lg shadow-lg h-full relative flex flex-col bg-[#1D2939] outline-none focus:outline-none'>
                <div className='grid grid-cols-3 max-md:grid-cols-2 gap-4 justify-items-center m-1 p-2 mb-0 pb-0 '>
