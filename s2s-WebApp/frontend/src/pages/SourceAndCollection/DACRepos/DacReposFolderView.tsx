...
interface DacReposNode {
  id: string
  name: string
  label: string
  icon?: string
  status?: string
  children?: DacReposNode[]
}

const DacReposFolderView = ({ setSelectId, selectId, dacFolderList, setDeleteimport, setUploadFile, setPage,
  setPageSize, }: any) => {
  ...
  const toggleNode = (id: string, params: any) => {
    setSelectId(params)
    setPage(0)
    setPageSize(10)
  ...
  const handleDelete = () => {
    let isDAC = true
    dispatch(FolderDelete(selectId.id, isDAC) as any)
      .then((res: any) => {
  ...
  const findParentHierarchyWithObjects = (
    data: DacReposNode[],
    childId: any,
    hierarchy: { id: any; node: DacReposNode }[] = []
  ): { id: any; node: DacReposNode }[] | null => {
  ...
  useEffect(() => {
    if (dacFolderList.length > 0 && dacFolderId) {
      const parent: any = findParentHierarchyWithObjects(dacFolderList, dacFolderId);
      const exandId: any = parent?.map((x: any) => {
        return x.id
      })
      const findObject: any = parent?.find((x: any) => x.id == dacFolderId)
      setSelectId(findObject?.node);
      setExpandedNodes(exandId);
      setDacFolderId(null)
    }
  }, [dacFolderList])

  const renderTree = (nodes: DacReposNode[], isRoot = false) => {
    return nodes?.map((node) => (
      <div key={node.id} className='ml-2'>
        <div
          className={`flex items-center gap-3 rounded-md  ${selectId?.id == node.id ? 'border border-[#ee7103]' : ''
            } p-2 cursor-pointer hover:bg-gray-700 items-center`}
          onClick={() => toggleNode(node?.id, node)}
        >
          {node?.children && (
            <>
          ...
          {/* *************************Delete Icon************************* */}
          {isRoot && node?.status == "ACTIVE" && (<div className='flex items-end ml-auto'>
            {/* <svg
          ...
          {isRoot && node?.status == "SYNCING" && (<div className="badge border-solid border-2 border-[#1570EF] rounded-xl px-2 py-1 flex items-center justify-start">
            <div className="text text-[#1570EF] text-xs font-medium">{'Syncing...'}</div>
          </div>)}
        </div>
  ...
  return (
    ...
    >
      {dacFolderList?.length > 0 && (<>
        {renderTree(dacFolderList, true)}
      </>)}
...
export default DacReposFolderView
