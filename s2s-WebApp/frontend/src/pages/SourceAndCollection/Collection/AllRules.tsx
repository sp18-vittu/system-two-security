    ...
    MuiCheckbox: {
      styleOverrides: {
        root: {
          ...
          cursor: "default", // Default cursor for the border area
          ...
          // Target the checkbox input for the pointer cursor
          "& input": {
            cursor: "pointer",
...
function AllRules({ ruledata, setdeletvale, collectionList, setActiveTab, reloading }: any) {
  const { id } = useParams()
  ...
  const { tbList, setTBList }: any = useData()
  ...
  const onViewimage = async (e: any, params: any, type: any, platformName: any) => {
    navigateTo(`/app/sigmaruleview/${singleparams?.id}`, {
      state: {
        vaultId: id,
  ...
  const hanleTBNavigate = (params: any) => {
    if (params.sourceType == 'THREAT BRIEF') {
      ...
      const collectionLists: any = JSON.parse(sessionStorage.getItem('collectionList') as any)
      const collectiondata: any = collectionLists?.find((x: any) => x.id == params.sourceId)
      localStorage.setItem('collectiondetails', JSON.stringify(collectiondata))
      navigateTo(`/app/collectionsigmarule/${collectiondata?.id}`, {
        state: { title: collectiondata?.name },
      })
    }
  }
        ...
        return (
          <>
            <div>
              ...
              >
                {' '}
                {params?.row?.datavault?.name ? params?.row?.datavault?.name : '-'}
              </span>{' '}
            </div>
          </>
        )
            ...
            <div>
              ...
              {params.row.ctiReport ? (
                ...
                <BootstrapTooltip title={params.row.ctiName} arrow placement='bottom'>
                  <div className='truncate-text cursor-pointer' onClick={() => handleNavigateCti(params.row)}>
                    <span>{params.row.ctiName ? params.row.sourceName : '-'}</span>
                  </div>
                </BootstrapTooltip>
              ) : (
                <BootstrapTooltip title={params.row.sourceName} arrow placement='bottom'>
                  <div className='truncate-text cursor-pointer' onClick={() => hanleTBNavigate(params.row)}>
                    <span>{params.row.sourceName ? params.row.sourceName : '-'}</span>
                  </div>
                </BootstrapTooltip>
              )}
            </div>
                    ...
                    <button
                      ...
                      onClick={(e) =>
                        onViewimage(
                          ...
                          params.row,
                          'view',
                          singleparams?.availableTargetQueries?.length > 0
                            ? singleparams?.availableTargetQueries[0]
                            : '',
                        )
                      }
                    >
  ...
  const handleDelete = () => {
    ...
    if (deleteFilesIds.length > 0 && !singleparams) {
      dispatch(CollectioMultipleDelete(0, deleteFilesIds) as any).then((res: any) => {
      ...
      dispatch(CollectionruleDelete(0, singleparams?.id) as any).then((res: any) => {
  ...
  const mainContentStyles = {
    ...
    height: 'calc(100vh - 40px - 212px - 36px - 32px - 24px - 42px - 24px)',
    overflow: 'scroll',
  }
