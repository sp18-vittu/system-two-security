...
function DetectionsDetails({ setModalOpen, setDialogOpen, detectionsList, setSelectedRows, filterdata, setFilterdata, dLoader, setDetectionOpen, promptSources, setCollectionorcti, workingpage, setRuleIndex, setCreateSelectedRows, setExSelectedRows }: any) {
  ...
  const handleSaves = () => {
    ...
    const selectedRows = (filterdata?.length > 0 ? filterdata : detectionsList).filter((row: any) => selectedIDs.has(row.id));
    setCreateSelectedRows([])
    setExSelectedRows(selectedRows)
  }
    ...
    <div className='flex flex-col gap-6 items-start justify-start w-full'>
      <div className='flex items-center justify-between w-full max-2xl:flex-col gap-4'>
        <div className='flex items-center gap-3 items-center'>
          <div className='text-white text-lg font-medium'>{`Existing Detections (${filterdata?.length > 0 ? filterdata?.length : detectionsList?.length > 0 ? detectionsList.length : 0
            })`}</div>
        </div>
            ...
            <>
              ...
              <div className='flex items-center gap-3'>
                ...
                <button
                  ...
                  className={`${'cursor-pointer'} flex items-center justify-center gap-2 border border-orange-500 rounded-lg px-4 py-2 shadow-sm`}
                >
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide w-full">
        {detectionsList && detectionsList?.length > 0 ? (<div className="space-y-4 w-full max-h-[800px] scrollbar-hide">
          {(filterdata?.length > 0 ? filterdata : detectionsList).map((dectaion: any, index: any) => {
            let parsedJSON = yaml.load(dectaion?.content)
            let Category: any = dectaion?.datavault?.category ? (dectaion?.datavault?.category == "CTI" ? "Public CTI Report" : dectaion?.datavault?.category) : dectaion?.source == "cti" ? "Public CTI Report" : null;
                      ...
                      <>
                        {state?.sourcesheaer === "collection" || promptSources?.includes("collection") ? (
                          ...
                          <>
                            {
                              dLoader ? (
                                <div className={`text-[#53b1fd] ${'text-left'} font-normal text-[16px] leading-[28px] w-[95%] break-words line-clamp-2 cursor-pointer`}>
                                  <span >{"-"}</span>
                                </div>
                              ) : (<>
                                {(Category && datavaultname || Category && !datavaultname) ? (
                                  <BootstrapTooltip title={dectaion?.ctiName ? dectaion?.ctiName : datavaultname ? datavaultname : dectaion?.title} placement='bottom'>
                                    <div
                                    ...
                                    >
                                      {Category && datavaultname ? (
                                        <>
                                          {Category == "Public CTI Report" && dectaion?.global ? (
                                            <span className="uppercase">{Category}</span>
                                          ) : (
                                            <>
                                              {isUUID(datavaultname) ? (
                                                <>
                                                  <span className="uppercase">{Category}</span>
                                                </>
                                              ) : (
                                                <>
                                                  <span className="uppercase">{Category}</span> : <span>{datavaultname}</span>
                                                </>
                                              )}
