...
function ChatWorkBench() {
  ...
  const statusChange = (option: any) => {
    setAnchorEl(null)
    let ctiId: any = option == 'CTI Report' ? 'cti' : option == 'Search Detections' ? 'sigma_search' : 'rule_agent'
    ...
    dispatch(ChatHistoryDetails(userIdchat, (id ? id : Number(dataid)), noOfprompts) as any).then((reponse: any) => {
      if (reponse) {
        ...
        for (let i = 0; i < mergedMessages?.length; i++) {
          ...
          if ((id || dataid) && !mergedMessages[i]?.question && mergedMessages[i]?.artifacts?.length > 0) {
        ...
        if ((id || dataid) && artifactsList[lastIndex]?.artifacts?.length > 0) {
          fetchartifactsAllData(artifactsList[lastIndex].artifacts)
        }
  ...
  }, [messages, ctiChat, detectionsList?.length > 0, wssstatus, id, dLoader]);
    ...
    <div onClick={handlefullView}>
      {id && (<div className="bg-[#1d2939] p-6 flex flex-row items-center justify-between relative overflow-hidden max-md:gap-4 flex-wrap" >
        ...
        <div className="flex flex-row gap-4 items-center flex-1 justify-center">
          ...
          <div className="flex flex-row gap-2 items-center">
            <div className="text-white text-xl font-medium max-sm:text-lg"> {selectedOption == 'sigma_search' ? "Discover Detection" : selectedOption == 'cti' ? 'Review Detection' : 'Create Detection'}</div>
          </div>
      ...
      >
        <div className='grid grid-cols-12 items-center gap-4'>
          <div
            className={`transition-all duration-500 ease-in-out bg-white shadow-md ${id && detectionsList?.length > 0 && ShowRule ? 'col-span-6 translate-x-0 max-lg:hidden' : 'col-span-12 translate-x-0'
              }`}
            style={{
              height: `${dynamicHeight}px`,
              width: '100%',
            ...
            <div className='grid grid-rows-[1fr_auto] h-full'>
              <div className='overflow-y-auto p-6'>
                <div className='grid grid-cols-12 items-center justify-center gap-4'>
                  {(detectionsList?.length == 0 || !ShowRule) && <div className='col-span-2 max-lg:col-span-1 flex items-center justify-center'></div>}
                  <div className={`${(detectionsList?.length == 0 || !ShowRule) ? 'col-span-8 max-lg:col-span-10' : 'col-span-12'} relative`}>
                    {!id && (
                                  ...
                                  <div className='flex justify-between'>
                                    <div>
                                      {dummy?.map((chat: any, key: any) => {
                                        const status =
                                          responce.focus == 'cti' ? 'CTI Report' : responce.focus == 'sigma_search' ? 'Search Detections' : 'Detection Agent'
                                        return (
                                          <>
                                      ...
                                      <div key={index}>
                                        <div className='flex justify-between pt-[40px] mb-2 '>
                                          ...
                                          <div className='flex justify-between'>
                                            <div>
                                              {dummy?.map((chat: any, key: any) => {
                                                const status = selectedOption == 'cti' ? 'CTI Report' : selectedOption == 'sigma_search' ? 'Search Detections' : 'Detection Agent'
                                                return (
                                                  <>
                                                    <div className='relative inline-block text-right '>
                                                      <button
              ...
              >
                <div className='box-border bg-[#48576c] flex flex-col items-start justify-start w-full h-full relative  overflow-hidden'>
                  ...
                  <button
                    className='p-[2px_6px] flex flex-row items-center justify-start self-stretch flex-shrink-0 relative bg-transparent border-none'
                    onClick={() => statusChange('Search Detections')}
                  >
                    <div className='rounded-[6px] p-[9px_10px] flex flex-row gap-[12px] items-center justify-start flex-1 relative'>
                      <div className='flex flex-row gap-[8px] items-center justify-start flex-1 relative'>
              ...
              <div className='p-4'>
                <div className='grid grid-cols-12 items-center gap-4 justify-center'>
                  {(!ShowRule || detectionsList?.length == 0) && <div className='col-span-2 flex items-center justify-center max-lg:col-span-1'></div>}
                  <div className={`${(!ShowRule || detectionsList?.length == 0) ? 'col-span-8 max-lg:col-span-10' : 'col-span-12'} relative`}>
                            ...
                            >
                              <div className='px-[2px] py-0 flex flex-row gap-0 items-center justify-center flex-shrink-0 relative'>
                                <div className='text-[#ee7103] text-left font-semibold text-sm leading-5 relative'>
                                  {selectedOption == 'cti' ? 'CTI Report' : selectedOption == 'sigma_search' ? 'Search Detections' : 'Create Detections'}
                                </div>
                              </div>
          ...
          {id && detectionsList?.length > 0 && ShowRule && (<div
...
const svgIcons: any = {
  ...
  'Search Detections': (color: string) => (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='16' height='16'
      viewBox='0 0 20 20'
