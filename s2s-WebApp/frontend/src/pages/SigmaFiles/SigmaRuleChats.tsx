...
function SigmaRuleChats() {
  ...
  const [validbutton, setValidbutton] = useState(true as any)
  const [ymltext, setYmlText] = useState('' as any)
  const [copyText, setCopyText] = useState(null as any)
  const { setSigmaViewwss, yamlChatOpen, setYamlChatOpen }: any = useData()
  ...
  const fetchSessiondetails = () => {
    const dataid: any = sessionStorage.getItem('schatid')
    if (dataid) {
  ...
  const fetchHistoryAllData = () => {
    ...
    const dataid: any = sessionStorage.getItem('schatid')
    dispatch(ChatHistoryDetails(userIdchat, Number(dataid), noOfprompts) as any).then(
      (reponse: any) => {
        if (reponse) {
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (event.key === 'Enter' && !event.shiftKey && !isSend) {
    ...
    if (!dataid) {
      dispatch(createChat(selectFiles, chatObj) as any).then((newChatResponse: any) => {
        if (newChatResponse.type === 'CREATE_CHAT_SUCCESS') {
          sessionStorage.setItem('schatid', newChatResponse?.payload?.id)
          // navigateTo(`/app/sigmarulechats/${newChatResponse.payload.id}`)
      ...
      if (!socket?.readyState) {
        const dataid: any = sessionStorage.getItem('schatid')
        connectWebSocket(dataid, state ? state : selectFiles)
          .then((wsResponse: any) => {
            if (wsResponse) {
              sendPrompt(datavalut, wsResponse)
            }
  ...
  const sendPrompt = (datavalut: any, webSocket: any) => {
    if (sendmessage.trim() || cancelChat) {

      if (sendmessage || cancelChat) {
        setRows(1)
        let parsedJSON = yaml.load(state.yamlText)
        let object: any = {
          message_id: uuidv4(),
          message: sendmessage,
          focus: 'rule_agent',
          "artifacts": [{
            ...
            "data": {
              "attachments": [{
                "file_id": parsedJSON?.id,
                "file_type": "application/x-yaml",
                "category": "sigma_rule",
                "file_path": state?.sigmadetail?.filePath
              }]
            }
          }]
        }
  ...
  const wssConnectionMethod = (chatId: any, selectedFiles: any) => {
    ...
    const sessionnams: any = sessionList.find((x: any) => x.id == dataid)
    let ws: any = null
  ...
  const handelValidation = () => {
    const jsonString = JSON.parse(sessionStorage.getItem('yaml') as any)
    const validation = {
      sigma_rule: createdetectionsList?.length > 0 ? createdetectionsList[0]?.content : jsonString,
    }
    dispatch(yamlFileValidation(validation) as any).then((res: any) => {
      if (res.type == 'VALIDATION_YAML_FILE_SUCCESS') {
        if (res?.payload.valid) {
          ...
          let parsedJSON = yaml.load(createdetectionsList[0]?.content)
          const blob = new Blob([createdetectionsList[0]?.content], { type: 'text/plain' })
  ...
  const handleNavigate = () => {
    ...
    sessionStorage.getItem('schatid')
  }
  ...
  const handlecopyToinbox = () => {
    ...
    dispatch(yamlFileValidation(validation) as any).then((res: any) => {
      if (res.type == 'VALIDATION_YAML_FILE_SUCCESS') {
        if (res?.payload.valid) {
          setYamlChatOpen(false)
          let parsedJSON = yaml.load(jsonString)
          ...
          dispatch(yamlFileUpdate(Number(sigmadetail?.datavault?.id), id, data) as any).then((res: any) => {
            if (res.type == 'UPDATE_YAML_FILE_SUCCESS') {
              ...
              setValidbutton(null)
              dispatch(
                CollectiondtoCollectionMultipleCopyPost(Number(sigmadetail?.datavault?.id), inboxList?.id, [
                  id,
                ]) as any,
              ).then((res: any) => {
  ...
  const handlecopyToCollection = (params: any) => {
    const jsonString = JSON.parse(sessionStorage.getItem('yaml') as any)
    const validation = {
      sigma_rule: jsonString,
    }
  ...
  const text =
    "Change author name to System rule Chat title: Detection of\nid: 7033498e-ba50-4372-965b-44391c039c26\nstatus: experimental\ndescription: Detects the presence of the specific keyword \"#FREEWAIFU\" in internal network communications, which may indicate communication or data exfiltration related to the Snowflake extortion campaign by Kiberphant0m.\nreferences:\n  - https://krebsonsecurity.com/2024/11/hacker-in-snowflake-extortions-may-be-a-u-s-soldier\nauthor: System Two Security\ndate: '2024-12-03'\ntags:\n  - attack.command-and-control\nlogsource:\n  product: zeek\n  service: http\ndetection:\n  uri: '*#FREEWAIFU*'\n  condition: uri\nfalsepositives:\n  - Legitimate discussions or references to the hashtag\nlevel: high\n"
  const [beforeWith] = text.split(' rule')

  const splitquestion = (text: any) => {
    const [beforeWith] = text.split(' with')
    return beforeWith
  }
  ...
  const ymalTexts = JSON.parse(sessionStorage.getItem('yaml') as any)

  return (
    <>
      <div className='bg-[#1d2939] p-6 flex flex-row items-center justify-between relative gap-3 flex-wrap'>
        {/* Back Button */}
        <div onClick={handleNavigate} className='flex flex-row items-center gap-3 justify-center max-md:gap-1'>
          <svg
          ...
          <div className='text-[#ee7103] font-semibold text-lg leading-7 max-md:!text-sm'>Back</div>
        </div>

        {/* Title */}
        <div className="flex flex-row gap-4 items-center max-lg:flex-1 max-lg:justify-center">

          <div className="flex flex-row gap-2 items-center">
            <div className="text-white text-xl font-medium max-sm:text-lg"> {'Create Detection'}</div>
          </div>

          <div
            className={`relative flex-shrink-0 ${"w-[130px] h-[31px] p-[4px]"
        ...
        {/* Buttons */}
        <div className='flex flex-row items-center gap-3 max-lg:w-full justify-end'>
          <button
            onClick={() => handleShowRule()}
            className='border border-[#ee7103] rounded-lg px-3.5 py-2 shadow-sm max-sm:py-1.5'
          >
            <div className='text-sm font-semibold text-white max-sm:text-[12px]'>
              {!ShowRule ? 'Show Rule' : 'Hide Rule'}
            </div>
          </button>
          <button
            disabled={createdetectionsList?.length > 0 ? false : true}
            onClick={() => handelValidation()}
            className={`bg-[#ee7103] rounded-lg px-3.5 py-2 shadow-sm max-sm:py-1.5 ${createdetectionsList?.length == 0 ? 'cursor-not-allowed opacity-50' : `cursor-pointer`
              }`}
          >
            <div className='text-sm font-semibold text-white'>Validate and Save</div>
          </button>
        </div>
      </div>
      ...
      >
        <div className='grid grid-cols-12 items-center gap-4'>
          {/* Left side content */}
          <div
            className={`transition-all duration-500 ease-in-out bg-white shadow-md ${ShowRule ? 'col-span-6 translate-x-0 max-md:hidden' : 'col-span-12 translate-x-0'
              }`}
            style={{
              height: `${dynamicHeight}px`,
              width: '100%',
            ...
            <div className='grid grid-rows-[1fr_auto] h-full'>
              ...
              <div className='overflow-y-auto p-4'>
                <div className='grid grid-cols-12 items-center gap-4'>
                  ...
                  {!ShowRule && <div className='col-span-3 flex items-center justify-center max-lg:col-span-1'></div>}
                  <div className={`${!ShowRule ? 'col-span-6 max-lg:col-span-10' : 'col-span-12'} relative`}>
                    {ctiChat?.length == 0 ? (
                      <>
                                          ...
                                          components={{
                                            ...
                                            code({
                                              ...
                                              if (match && match[1] === 'yaml') {
                                                setYamlChatOpen(false);
                                                sessionStorage.setItem('yaml', JSON.stringify(codeContent));
                                              }
                                                  ...
                                                  <div className='flex items-center justify-between  w-full h-full flex-wrap py-2'>
                                                    ...
                                                    <div className='flex items-center gap-6 justify-end flex-wrap'>
                                                      ...
                                                      <div className='flex items-center justify-center gap-1.5 cursor-pointer' onClick={() => copyToClipboard(codeContent)}>
                                                        ...
                                                        {showPopover && (
                                                          <div className='absolute items-start justify-center  p-1 bg-white text-black rounded shadow z-10 mt-16'>
                                                            Copied!
                                                          </div>
                                                        )}
                                                      </div>
                                                      ...
                                                      <div className='flex items-center justify-center gap-1.5 cursor-pointer' onClick={() => handlecopyToinbox()}>
                                                        <span className='text-[#EE7103] text-sm font-semibold leading-[20px] font-inter'>
                                                      ...
                                                      <div className='flex items-center justify-center gap-1.5 cursor-pointer' onClick={() =>
                                                        handlecopyToCollection('chatcopy')
                                                      }>
                                                        <span className='text-[#EE7103] text-sm font-semibold leading-[20px] font-inter'>
          ...
          {ShowRule && (
            <div
              className={`transition-all duration-500 ease-in-out   ${ShowRule
                ? 'col-span-6  translate-x-0 opacity-100 max-md:col-span-12'
            ...
            >
              ...
              <div className='overflow-y-auto p-4 h-full'>
                <YamlEditor
                  ymltext={createdetectionsList?.length > 0 ? createdetectionsList[0]?.content : state?.yamlText}
                  setYmlText={setYmlText}
                  setSeloctror={setCopyText}
                  modeOfView={'ruleeditorsss'}
                />
              </div>
