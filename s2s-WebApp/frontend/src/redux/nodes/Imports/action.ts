...
export const ImportRuleViewList = (folderid: any, object: any) => async (dispatch: any, getState: any) => {
  const localStorage = local.getItem('bearerToken')
  const token = JSON.parse(localStorage as any)
  dispatch({ type: IMPORT_RULE_GET_REQUEST })
  try {
    const { data } = await api.get(`/data/imported-source/${folderid}/rule-paged`, {
      headers: { Authorization: `${token.bearerToken}`, "Content-Type": "application/json", },
      params: {
        asc: object.isDAC,
        offset: object.offset, // API expects 1-based index
