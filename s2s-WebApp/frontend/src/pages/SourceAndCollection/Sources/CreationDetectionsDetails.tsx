...
function CreationDetectionsDetails({ setModalOpen, setDialogOpen, detectionsList, setSelectedRows, filterdata, setFilterdata, dLoader, setDetectionOpen, promptSources, setCollectionorcti, workingpage, setRuleIndex, isAccordionOpen, setIsAccordionOpen, setCreateSelectedRows, setExSelectedRows }: any) {
    ...
    const [openAccordionId, setOpenAccordionId] = useState([detectionsList[0]?.id] as any);
    ...
    const toggleAccordion = (id: any) => {
        if (openAccordionId.includes(id)) {
            ...
            setIsAccordionOpen(openAccordionId.filter((nodeId: any) => nodeId !== id))
        } else {
            setOpenAccordionId([...openAccordionId, id])
            setIsAccordionOpen([...openAccordionId, id])
        }
    }
    ...
    const handleSaves = () => {
        ...
        const selectedRows = (filterdata?.length > 0 ? filterdata : detectionsList).filter((row: any) => selectedIDs.has(row.id));
        setCreateSelectedRows(selectedRows)
        setExSelectedRows([])
    }
        ...
        <div className='flex flex-col gap-6 items-start justify-start w-full'>
            <div className='flex items-center justify-between w-full max-2xl:flex-col gap-4'>
                <div className='flex items-center gap-3 items-center'>
                    <div className='text-white text-lg font-medium'>{`Created Detection (${filterdata?.length > 0 ? filterdata?.length : detectionsList?.length > 0 ? detectionsList?.length : 0})`}</div>
                </div>
            ...
            {detectionsList && detectionsList?.length > 0 ? (<div className="space-y-4 w-full overflow-y-auto  max-h-[600px]">
                {(filterdata?.length > 0 ? filterdata : detectionsList).map((dectaion: any, index: any) => {
                    let parsedJSON = yaml.load(dectaion?.content)
                    let Category: any = dectaion?.datavault?.category ? (dectaion?.datavault?.category == "CTI" ? "Public CTI" : dectaion?.datavault?.category) : dectaion?.source == "cti" ? "Public CTI" : 'S2S Generated';
                                                ...
                                                <>
                                                    {Category && datavaultname ? (
                                                        ...
                                                        <div onClick={() => handleClickOpen(dectaion?.id, index)} className={`text-[#53b1fd] ${'text-left'} font-normal text-[16px] leading-[28px] w-[95%] break-words line-clamp-2 cursor-pointer`}>
                                                            <span className="uppercase">{"S2S Generated"}</span>
                                                        </div>
                                                    )}
                                                </>
                                            )}
...
export default CreationDetectionsDetails
