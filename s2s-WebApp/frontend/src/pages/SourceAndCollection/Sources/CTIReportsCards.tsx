...
function CTIReportsCards({
  ...
  return (
    <div style={ width > 768 ? mainContentStyles : normalStyles}
      className='grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 w-full scrollbar-hide'
    >
        ...
        <>
          {(sortedDataLists?.length > 0 ? sortedDataLists : cardList)?.map(
            (item: any, index: any) => (
              <div
                key={index}
                className='flex flex-col items-start justify-between h-full border border-[#3E4B5D] bg-[#1D2939] rounded-[10px] cursor-pointer'
              ...
              >
                ...
                <div className='flex items-center justify-between p-4 md:p-6 gap-6 w-full bg-[#101828] border-t border-[#3E4B5D] rounded-b-[10px]'>
                  {item?.status === 'COMPLETE' ? (
                    <>
