...
import { ImportFolderdataPost } from '../../../redux/nodes/Imports/action'
...
const NewFolderModal: React.FC<NewFolderModalProps> = ({
  ...
  const onSubmit = (data: any) => {
    if (selectFolders && cardList?.length > 0) {
      data.name = selectFolders?.label + '/' + data.name
      if (
        cardList.some(
          (collection: any) =>
            collection.label.toLowerCase() === data?.label?.trimEnd().toLowerCase(),
        )
      ) {
        setError('name', {
          type: 'manual',
        ...
        dispatch(ImportFolderdataPost(postdata) as any).then((response: any) => {
          if (response.type === 'IMPORT_TREEVIEW_POST_SUCCESS') {
            response.payload.label = response.payload.name
            setSelectFolders(response?.payload)
            setSelectId(response?.payload)
            toast.success(
              <CustomToast
  ...
  return (
    <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50'>
      ...
      >
        ...
        <h2 className='text-xl font-semibold mb-6'>New Folder</h2>
        <div className='space-y-6'>
          ...
          <div>
            ...
            <textarea
              id='description'
              placeholder='Describe the Folder (Optional)'
              maxLength={128}
        ...
        <div className='flex justify-end space-x-2 mt-8 absolute bottom-6 right-6'>
          ...
          <button
            ...
            type='submit'
            className={`px-6 py-1 bg-orange-500 text-white rounded-md hover:bg-orange-600 transition ${!!errors.name ? 'cursor-not-allowed opacity-50' : ''
              }`}
          >
            Save
          </button>
