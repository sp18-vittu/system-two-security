...
import { Box, IconButton, Menu, styled, Tooltip, tooltipClasses, TooltipProps } from '@mui/material'
...
import { importRuleDelete } from '../../../redux/nodes/Imports/action'
...
function DacReposTable({ selectId, dacfoldersList, setUploadFile, isLoading,
  page,
  setPage,
  pageSize,
  setPageSize,
  ...
  const handleOpenInboxCollection = (datas: any) => {
    if (datas == 'multipleInbox') {
      ...
      if (inboxList) {
        dispatch(
          CollectiondtoCollectionMultipleCopyPost(selectId?.id, [inboxList?.id], getId) as any,
        ).then((res: any) => {
    ...
    if (getCheckedValue.length > 0) {
      ...
      try {
        ...
        reader.onload = function (e) {
          ...
          downloadLink.download = `S2S_${selectId.name}.zip`
          downloadLink.click()
  ...
  const handleDelete = () => {
    ...
    if (deleteFilesIds.length > 0 && !singleparams) {
      let isDAC = true
      dispatch(importRuleDelete(selectId?.id, deleteFilesIds, isDAC) as any).then((res: any) => {
        if (res.type == 'IMPORT_RULE_DELETE_SUCCESS') {
          toast.dismiss(toastId)
          toast.success(
            <CustomToast
              message={
      ...
      let isDAC = true
      dispatch(importRuleDelete(selectId?.id, [singleparams?.id], isDAC) as any).then(
        (res: any) => {
          if (res.type == 'IMPORT_RULE_DELETE_SUCCESS') {
            toast.dismiss(toastId)
            toast.success(
  ...
  const columns: GridColDef[] = [
            ...
            {params?.row?.source == 'FACTORY_SIGMAHQ' ? (
              <>
                <BootstrapTooltip title={'Sigma HQ'} arrow placement='bottom'>
                  ...
                  >
                    <path
                      d='M10.9709 24C17.5735 24 23.3867 18.6274 23.9551 12C24.5234 5.37258 19.6317 0 13.0291 0C6.4265 0 0.613278 5.37258 0.0449157 12C-0.523447 18.6274 4.36828 24 10.9709 24Z'
                      fill='#204D7B'
                    />
                    <path
                      d='M22.6615 4.21289C22.7384 4.86624 22.5278 5.49531 22.0298 6.10011C21.5319 6.70492 20.8615 7.00732 20.0186 7.00732L18.4157 7.0166C19.4183 8.36415 19.9422 10.097 19.78 11.9874C19.4116 16.2811 15.643 19.7618 11.3627 19.7618C7.08234 19.7618 3.91115 16.2811 4.27961 11.9874C4.64806 7.69364 8.41664 4.21289 12.697 4.21289H22.6615ZM12.4534 7.05119C9.73573 7.05119 7.34299 9.26119 7.10905 11.9874C6.87511 14.7135 8.88856 16.9235 11.6062 16.9235C14.3239 16.9235 16.7166 14.7135 16.9506 11.9874C17.1845 9.26119 15.1711 7.05119 12.4534 7.05119Z'
                      fill='#24D2FF'
                    />
                    <path
12.3381 81.3008 12.2244C81.4854 12.1108 81.5778 11.9261 81.5778 11.6705V11.6207C81.5778 11.0005 81.4026 10.5199 81.0522 10.179C80.7066 9.83807 80.1905 9.66761 79.5039 9.66761C78.7889 9.66761 78.2255 9.82623 77.8136 10.1435C77.4064 10.456 77.1246 10.804 76.9684 11.1875L74.9727 10.733C75.2094 10.0701 75.555 9.53504 76.0096 9.12784C76.4689 8.71591 76.9968 8.41761 77.5934 8.23295C78.19 8.04356 78.8174 7.94886 79.4755 7.94886C79.9111 7.94886 80.3728 8.00095 80.8604 8.10511C81.3529 8.20455 81.8121 8.3892 82.2383 8.65909C82.6692 8.92898 83.0219 9.31487 83.2965 9.81676C83.5711 10.3139 83.7085 10.9602 83.7085 11.7557V19H81.6346V17.5085H81.5494C81.4121 17.7831 81.2061 18.053 80.9315 18.3182C80.6568 18.5833 80.3041 18.8035 79.8732 18.9787C79.4424 19.1539 78.9263 19.2415 78.3249 19.2415ZM78.7866 17.5369C79.3737 17.5369 79.8756 17.4209 80.2923 17.1889C80.7137 16.9569 81.0333 16.6539 81.2511 16.2798C81.4736 15.901 81.5849 15.4962 81.5849 15.0653V13.6591C81.5091 13.7348 81.3623 13.8059 81.1445 13.8722C80.9315 13.9337 80.6876 13.9882 80.413 14.0355C80.1384 14.0781 79.8709 14.1184 79.6104 14.1562C79.35 14.1894 79.1322 14.2178 78.957 14.2415C78.5451 14.2936 78.1687 14.3812 77.8278 14.5043C77.4916 14.6274 77.2217 14.8049 77.0181 15.0369C76.8192 15.2642 76.7198 15.5672 76.7198 15.946C76.7198 16.4716 76.9139 16.8693 77.3022 17.1392C77.6905 17.4044 78.1853 17.5369 78.7866 17.5369Z'
                      fill='white'
                    />
                  </svg>
                </BootstrapTooltip>
    ...
    },
    {
      ...
      headerClassName: 'hideRightSeparator',
      renderCell: (params: any) => {
        const validdetails: any = JSON.parse(params.row.errors)
        return (
          <>
            {validdetails ? (
              <>
                {validdetails?.valid && (<div className="badge border-solid border-2 border-[#16A34A] rounded-xl px-2 py-1 flex items-center justify-start">
                  <div className="text text-[#16A34A] text-xs font-medium">{'Valid'}</div>
                </div>)}
                {!validdetails?.valid && (
                  <BootstrapTooltip title={validdetails?.errors[0]} placement='bottom'>
                    <div className="badge border-solid border-2 border-[#D92D20] rounded-xl px-2 py-1 flex items-center justify-start">
                      <div className="text text-[#D92D20] text-xs font-medium">{'Invalid'}</div>
                    </div>
                  </BootstrapTooltip>
                )}
              </>
        ...
        <>
          <div className='flex justify-between w-full '>
            {params.row.uploadDatetime ? (
              <>
                <span>{moment(params.row.uploadDatetime).format('DD-MMM HH:mm')}</span>
              </>
            ) : (
              <>{'-'}</>
            )}
    ...
    //   flex: 0.4,
    //   headerClassName: 'hideRightSeparator',
    //   renderCell: (params:any) => <div className='flex justify-between w-full '>{'-'}</div>,
    // },
    // {
    //   field: 'link',
    //   headerName: 'LINK',
    //   flex: 1,
    //   headerClassName: 'hideRightSeparator',
    //   renderCell: (params) => <div className='flex justify-between w-full '>{'-'}</div>,
    // },
    {
      field: 'menu',
      headerName: '',
      ...
      renderCell: (params: any) => (
        <>
          <IconButton>
            <MoreVertIcon
              style={{ color: 'white' }}
              onClick={(e: any) => handleClickdot(e, params.row)}
            />
          </IconButton>
          <Menu
            sx={{
  ...
  const filterdata = dacfoldersList?.filter(
    (collection: any) =>
      collection?.title?.toLowerCase().includes(search?.toLowerCase()) ||
      collection?.name?.toLowerCase().includes(search?.toLowerCase()),
  )

  const label: any = selectId?.label ? 'All Files/' + selectId?.label : 'All Files'
  const parts: any = selectId?.label ? label.split('/') : label

  const handleRefresh = () => {
    setUploadFile('add')
    setIsRefreshing(true);
    setTimeout(() => setIsRefreshing(false), 500); // Reset after animation
  };

  return (
    <div className='bg-[#1d2939] rounded-lg p-8 flex flex-col gap-6 items-start justify-start w-full h-full overflow-scroll hide-scrollbar'>
      {/* Breadcrumb Section */}
            ...
            {parts.map((part: any, index: any) => (
              <span key={index}>
                ...
                {index < parts.length - 1 && (
                  <span
                    className='text-white font-inter font-semibold text-[20px] leading-[24px] px-1 max-md:!text-sm'
                    style={{ overflowWrap: 'anywhere' }}
                  >
                    /
                  </span>
      ...
      <div className='flex justify-between w-full items-center gap-4 flex-wrap'>
        ...
        <div className='flex flex-row gap-2 flex-1 min-w-[200px]'>
          <div className='relative flex items-center border border-[#6e7580] rounded-lg bg-[#48576c] w-full h-9 shadow-xs'>
            <input
              type='text'
              placeholder='Search'
              className='w-full h-full px-3 bg-transparent text-white outline-none rounded-l-lg'
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            <button className='absolute right-0 h-full w-10 flex justify-center items-center bg-transparent rounded-r-lg'>
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='20'
                height='20'
          ...
          {/* Sort Button */}
          {/* <button className="flex items-center gap-2 px-4 h-9 border border-[#6e7580] bg-[#48576c] rounded-lg text-white font-semibold shadow-xs">
                        <span>Sort By</span>
                        <img className="w-5 h-5" src="arrows-down0.svg" alt="Sort" />
                    </button> */}
        </div>
        <div className='flex items-center gap-4 flex-1 whitespace-nowrap justify-end max-sm:flex-wrap max-sm:basis-full'>
          ...
          <svg
            ...
            fill='none'
          >
            <path d='M1 1V37' stroke='#475467' stroke-linecap='round' />
          </svg>
          <button
          ...
          <button
            ...
            className='flex items-center gap-2 py-2  text-white rounded-md shadow-sm'
          >
      ...
      <CopyAndNewCollectionsDialog
        ...
        importId={selectId?.id}
      />
      <DeleteConfirmationDialog
        isOpen={isdelDialogOpen}
        onClose={() => {
          setIsdleDialogOpen(false), setSingleparams(null), setUploadFile(null)
        }}
