...
function ImportsFileTable({
  ...
  const label: any = selectFolders?.label ? 'All Files/' + selectFolders?.label : 'All Files'
  const parts: any = selectFolders?.label ? label.split('/') : label
    ...
    if (getCheckedValue.length > 0) {
      ...
      try {
        ...
        reader.onload = function (e) {
          ...
          downloadLink.download = `S2S_${selectFolders.name}.zip`
          downloadLink.click()
  ...
  const handleDelete = () => {
    ...
    if (deleteFilesIds.length > 0 && !singleparams) {
      let isDAC = false
      dispatch(importRuleDelete(selectFolders?.id, deleteFilesIds, isDAC) as any).then(
        (res: any) => {
      ...
      let isDAC = false
      dispatch(importRuleDelete(selectFolders?.id, [singleparams?.id], isDAC) as any).then(
        (res: any) => {
          if (res.type == 'IMPORT_RULE_DELETE_SUCCESS') {
            toast.dismiss(toastId)
            toast.success(
  ...
  const handleOpenInboxCollection = (datas: any) => {
    if (datas == 'multipleInbox') {
      ...
      if (inboxList) {
        dispatch(
          CollectiondtoCollectionMultipleCopyPost(selectFolders?.id, [inboxList?.id], getId) as any,
        ).then((res: any) => {
  ...
  const onTitleViewimage = async (e: any, params: any, type: any) => {
    navigateTo(`/app/sigmafilesview/${params?.id}`, {
      state: {
        ...
        sigmadetail: params,
        title: selectFolders.name,
        platformName: 'view',
        paramsdata: 'Import',
        singleparams: params,
      },
    })
    setImpFolderId(selectFolders)
  }
        ...
        return (
          <>
            {validdetails ? (
              ...
              <div className="badge border-solid border-2 border-[#D92D20] rounded-xl px-2 py-1 flex items-center justify-start">
                <div className="text text-[#D92D20] text-xs font-medium">{'Processing...'}</div>
              </div>
            )}
          </>
        )
  ...
  const filterdata: any = foldersList.filter((collection: any) =>
  ...
  return (
    <div className='bg-[#1d2939] rounded-lg p-8 flex flex-col gap-6 items-start justify-start w-full h-full overflow-scroll hide-scrollbar'>
      ...
      <div className='flex justify-between items-center w-full gap-4 flex-wrap'>
        <div className='flex items-center gap-4  w-[65%] flex-1 min-w-[200px]'>
          <div className='flex items-center gap-2 px-2 py-1 h-9 bg-white shadow-sm border border-gray-300 rounded-md w-full'>
            <input
              type='text'
              placeholder='Search'
              className='flex-1 text-sm text-gray-500 bg-transparent outline-none w-[75%]'
              value={search}
        ...
        <div className='flex items-center gap-4 flex-1 whitespace-nowrap justify-end max-sm:flex-wrap max-sm:basis-full'>
          ...
          <button
            onClick={() => (setImportFolders(true), setFoldersList([]))}
            className='flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-md shadow-sm'
          >
...
export default ImportsFileTable
