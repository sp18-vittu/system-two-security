...
function SourceRuleChats() {
    ...
    const fetchartifactsAllData = async (fetchdata: any) => {
        ...
        if (fetchdata && (fetchdata[0]?.data?.sigmaRules?.length > 0 || fetchdata[0]?.data?.sigma_rules?.length > 0)) {
            const docId: any = (fetchdata[0]?.data?.sigmaRules?.length > 0 ? fetchdata[0]?.data?.sigmaRules : fetchdata[0]?.data?.sigma_rules).map((row: any) => {
                return row?.id
            });
            await dispatch(documentGetuuid(docId) as any).then((res: any) => {
                if (res.type == "DOCUMENT_UUID_GET_SUCCESS") {
                    ...
                    const updatedArray2 = res?.payload?.map((item: any, index: any) => {
                        const match = (fetchdata[0]?.data?.sigmaRules?.length > 0 ? fetchdata[0]?.data?.sigmaRules : fetchdata[0]?.data?.sigma_rules).find((el: any) => el?.id === item?.uuid);
                        if (match) {
                            return {
                                ...item,
                                source: match.source,
                            ...
                            let parsedJSON = fetchdata[0]?.data?.sigmaRules?.length > 0 ? yaml.load(fetchdata[0]?.data?.sigmaRules[index]?.content) : yaml.load(fetchdata[0]?.data?.sigma_rules[index]?.content)
                            if (fetchdata[0]?.data?.sigmaRules?.length > 0) {
                                return {
    ...
    }, [messages, ctiChat, detectionsList?.length > 0, wssstatus]);
        ...
        <>
            ...
            >
                <div className='grid grid-cols-12 items-center gap-4'>
                    <div
                        className={`transition-all duration-500 ease-in-out bg-white shadow-md ${detectionsList?.length > 0 && ShowRule ? 'col-span-5 translate-x-0 max-lg:hidden' : 'col-span-12 translate-x-0'
                            }`}
                        style={{
                            height: `${dynamicHeight}px`,
                            width: '100%',
                        ...
                        <div className='grid grid-rows-[1fr_auto] h-full'>
                            ...
                            <div className='overflow-y-auto p-4 min-h-full'>
                                <div className='grid grid-cols-12 items-center gap-4'>
                                    ...
                                    {(detectionsList?.length == 0 || !ShowRule) && <div className='col-span-3 flex items-center justify-center max-lg:col-span-1'></div>}
                                    <div className={`${(detectionsList?.length == 0 || !ShowRule) ? 'col-span-6 max-lg:col-span-10' : 'col-span-12'} relative`}>
                    ...
                    {detectionsList?.length > 0 && ShowRule && (
                        <div
