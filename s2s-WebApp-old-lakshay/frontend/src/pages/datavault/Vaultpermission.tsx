...
import { CREATE_NEW_DATAVALUT_RESET } from '../../redux/nodes/datavault/action'
import {
  SIGMA_FILE_DELETE_ALL_RESET,
  SIGMA_FILE_DELETE_RESET,
  sigmaCitNameList,
  sigmaFileAllsingleDelete,
  sigmaFileList,
  sigmaFilesingleDelete,
} from '../../redux/nodes/sigma-files/action'
import { VALUTPERMISSIONROLE_ROLE_UPDATE_RESET } from '../../redux/nodes/vaultPermission/action'
import local from '../../utils/local'
import { useData } from '../../layouts/shared/DataProvider'
import {
  BulkTranslateFileList,
...
import { sigmaFilter } from '../../redux/nodes/overviewSigma/action'
import YamlEditorForView from './YamlEditorForView'
import YamlTextEditorForQuery from './YamlTextEditorForQuery'
...
import InspectSectionView from './InspectSectionView'
...
const VaultPermission = () => {
  ...
  const { data, copyFiles, setData, detail, setDetail, setCopyFiles }: any = useData()
  ...
  const { vaultPermissionDetails, loading, error, success } = vaultPermissionDetail
  const vaultPermissionRoleDetail = useSelector(
    (state: any) => state.vaultPermissionroleDetailsreducer,
  )
  const { vaultPermissionroleDetails } = vaultPermissionRoleDetail
  const saveuser = useSelector((state: any) => state.saveuserreducer)
  const { success: savesuccess } = saveuser
  const updateuser = useSelector((state: any) => state.vaultPermissionaceessupdatereducer)
  const { success: updatesuccess } = updateuser
  const [vaultPermissionRoleData, setvaultPermissionRoleData] = useState([] as any)
  const deleteuser = useSelector((state: any) => state.sigmadeletereducer)
  const { success: removesuccess } = deleteuser

  const sigmaalldelete = useSelector((state: any) => state.sigmaalldeletereducer)
  const { success: removeAllsuccess } = sigmaalldelete

  const { success: uploadSuccess } = useSelector((state: any) => state.dataIngestionereducer)

  const Token = local.getItem('bearerToken')
  const token = JSON.parse(Token as any)

  if (vaultPermissionDetails || vaultPermissionRoleData) {
    sessionStorage.setItem('vaultPermissionDetails', JSON.stringify(vaultPermissionDetails))
    sessionStorage.setItem('vaultPermissionRoleData', JSON.stringify(vaultPermissionRoleData))
  }

  const [vaultdeleteAll, SetvaultdeleteAll] = useState([])
  const handleDeleteMultipleFiles = () => {
    const deleteFilesIds: any = deleteall.map((row: any) => {
      return { id: row.id }
    })
    SetvaultdeleteAll(deleteFilesIds)
  ...
  const vaultdeletefinal = () => {
    if (vaultdeleteAll.length > 0) {
      dispatch(sigmaFileAllsingleDelete(token, id, vaultdeleteAll) as any)
      setShowModalRemove(false)
      setGetCheckedValue([])
    } else {
      dispatch(sigmaFilesingleDelete(token, singleparams?.id) as any)
      setShowModalRemove(false)
    }
    fetchdetails()
  }
  // ***************************Over All Editor Section***************************
  ...
  const onViewimageTargt = async (
    ...
    dispatch(SingleTranslateFileList(obj, params) as any).then((response: any) => {
      if (response.payload.query) {
        ...
        setSingleResponse(response.type)
  ...
  const queryExecute = false
  const [vaultList, setVaultList] = useState(null as any)
  ...
  const onTitleViewimage = async (e: any, params: any, type: any) => {
    navigateTo(`/app/sigmafilesview/${params?.id}`, {
      state: { vaultId: id, singmaname: params.title, sigmadetail: params, platformName: null },
    })
  }
  const ctiSectionId: any = []
  const inspectPage = 'VaultPermission'
  ...
  const shareFile = (event: any, arr: any, value: string) => {
    ...
    setCopyFiles({ from: 'vaultPermission', value: { selectSigma: arr, mode: value } })
  }
  ...
  const bulckTranslateDownload = async () => {
    try {
      ...
      const { data } = await Axios.post(`${environment.baseUrl}/data/pysigma/download-query`, obj, {
        ...
        params: { global: vaultdata.global },
      })
      var reader = new FileReader()
      reader.onload = function (e) {
  ...
  const [singleeResponse, setSingleResponse] = useState(null as any)
  ...
  const handleClickTargersingle = (e: any) => {
    if (e.target.value) {
      const data: any = filterState?.find(
        (x: any) => x.id == singleparams.id,
      )?.availableTargetQueries
      const findvalue = data?.find((x: any) => x == e.target.value.toLowerCase())
      if (findvalue) {
        let obj = {
  ...
  useEffect(() => {
    if (copyFiles?.vaultList != 'pastsigmafile' || !copyFiles) {
      if (removesuccess) {
        dispatch({ type: SIGMA_FILE_DELETE_RESET })
        fetchdetails()
      }

      if (removeAllsuccess) {
        dispatch({ type: SIGMA_FILE_DELETE_ALL_RESET })
        fetchdetails()
      }
      if (savesuccess) {
      ...
      if (updatesuccess) {
        dispatch({ type: VALUTPERMISSIONROLE_ROLE_UPDATE_RESET })
      }
      if (uploadSuccess) {
        fetchdetails()
      }
    }
    if (copyFiles?.vaultList == 'pastsigmafile') {
      fetchdetails()
    }
  }, [removesuccess, removeAllsuccess, savesuccess, updatesuccess, uploadSuccess, id])

  useEffect(() => {
    setvaultPermissionRoleData(vaultPermissionroleDetails)
  }, [vaultPermissionDetails, vaultPermissionroleDetails])
  ...
  const deleteall = selectedRows.filter((x: any) => x.source == 'MANUAL_UPLOAD')
  ...
  // ******************** api Integration*************************
  const dataSingmaFiles = useSelector((state: any) => state.dataSingmaFilesreducer)
  const { singmaFiles } = dataSingmaFiles

  const [anchorEl0, setAnchorEl0] = useState(null)
  const [filterTitle, setFilterTitle] = useState(null as any)
  ...
  const [filterValueOfTitle, setFilterValueOfTitle] = useState(null as any)
  const [filterValueOfSource, setFilterValueOfSource] = useState(null as any)

  const [selectTargers, setSelectTargers] = React.useState([])

  let arraysource: any = [
    { title: 'GENERATED', name: 'GENERATED' },
    { title: 'UPLOADED', name: 'MANUAL_UPLOAD' },
    { title: 'SIGMAHQ', name: 'FACTORY_SIGMAHQ' },
    { title: 'MBABINSKI', name: 'FACTORY_MBABINSKI' },
  ]

  const fetchdetails = () => {
    dispatch(sigmaFileList(token, id) as any).then((data: any) => {
  ...
  const handleQueryExecute = async (e: any) => {
    setDisable(true)
    setclosePopupDisable(true)
    try {
      await dispatch(
        getExecuteQuery({ target: plateFormName, query: queryValue?.toString() }) as any,
      )
        .then((res: any) => {
  ...
  const handleClickMenu = (event: any, value: any) => {
    dispatch(sigmaCitNameList(token, value) as any).then((data: any) => {
      setsearchValue(data.payload)
      setFilterState(data.payload)
      setFilterValueOfTitle(value)
      setFilterValueOfSource(null)
      setAnchorEl0(null)
      setAnchorEl(null)
      setmenuValue('')
    })
  ...
  const handleClickMenusource = (event: any, value: any) => {
    filterArray = []
    singmaFiles
      .filter((item: any) => {
        return item.source == value?.name
      })
      .map((item: any) => {
        filterArray.push(item)
      })
    setsearchValue(filterArray)
    setFilterState(filterArray)
    setFilterValueOfSource(value)
    setFilterValueOfTitle(null)
    setAnchorEl0(null)
    setAnchorEl(null)
    setmenuValue('')
  }

  const handleMenuOpen0 = (event: any, title: any) => {
    setFilterTitle(title)
    setAnchorEl0(event.currentTarget)
  }
  const handleMenuClose0 = () => {
    setAnchorEl0(null)
  }
                ...
                <ul>
                  ...
                  <>
                    <button className='flex'>
                      <ShareOutlinedIcon style={{ width: 20, height: 20 }} />
                      <li className='text-sm font-medium ml-3'>Share</li>
                    </button>
                  </>

                  {singleparams?.source === 'MANUAL_UPLOAD' && (
                    <button
                      type='button'
                      className='flex'
                    ...
                    >
                      ...
                      <span className='m-1 ml-3  text-sm font-medium'> Delete</span>
                    </button>
                  )}
                </ul>
              </div>
  ...
  const VaultDropdown = () => {
    {
      SampleFilter ? setSampleFilter(false) : null
    }
    {
      Samplesort ? setSamplesort(false) : null
    }
  }
  ...
  const onSearch = (e: any) => {
    ...
    data = data?.filter((value: any) => value?.title?.toLowerCase().includes(query?.toLowerCase()))
    setFilterState(data)
    setmenuValue(query)
  }
  ...
  const [getCheckedDeleteValue, setGetCheckedDeleteValue] = useState([])
  ...
  useEffect(() => {
    if (data !== 'FILE_UPLOADED' && data?.report) {
      dispatch(sigmaFilter(data?.report?.ctiReportId) as any)
        .then((res: any) => {
          setFilterState(res.payload)
        })
        .catch((err: any) => console.log('err', err))
    } else if (data !== 'FILE_UPLOADED' && data?.params) {
      const { params } = data
    } else {
    }
  }, [data])
  ...
  useEffect(() => {
    if (
      detail &&
      (detail.from == 'RepositoryInspectSigmas' ||
        detail.from == 'Dashboard' ||
        detail.from == 'InsightInspectSigmas')
    ) {
      let demoObject = {
        id: id,
        global: detail?.value?.global,
      }
      sessionStorage.setItem('vault', JSON.stringify(demoObject))
      dispatch(sigmaCitNameList(token, detail.value) as any).then((res: any) => {
        if (res.type == 'CIT_NAME_LIST_SUCCESS') {
          setsearchValue(res.payload)
          setFilterState(res.payload)
          setFilterValueOfTitle(detail.value)
          setFilterTitle('filter_by_title')
          setAnchorEl0(null)
          setAnchorEl(null)
          setDetail(null)
        } else {
  ...
  const [ctiReportFiles, setctiReportFiles] = useState([] as any)
  ...
  useEffect(() => {
    ...
    dispatch(ctiReportFileList(token, selectedVault) as any).then((res: any) => {
      setctiReportFiles(res?.payload)
    })
  }, [])
    ...
    <>
      <div className='bg-[#0C111D]'>
        <div className=' focus:bg-white '>
          <div className='lg:relative'>
            <div className='lg:inline-flex'></div>
          </div>
          <nav
            className='focus:bg-white coolGray-100 flex-no-wrap relative flex w-full items-center justify-between bg-[#0C111D] py-2 shadow-md shadow-black/5 lg:flex-wrap lg:justify-start lg:pb-4 lg:pt-0'
            onClick={VaultDropdown}
          >
            <div className='flex w-full flex-wrap items-center justify-between px-2 bg-[#0C111D]'>
              <div
                className='flex-grow basis-[100%] items-center lg:!flex lg:basis-auto sm:block'
              ...
              >
                <div className='relative' data-te-dropdown-ref data-te-dropdown-alignment='end'>
                  ...
                  >
                    ...
                    <li className='mb-4 lg:mb-0 lg:pr-2' data-te-nav-item-ref>
                      <a
                        className='cursor-pointer text-white font-semibold text-sm leading-5 underline underline-offset-8 transition duration-200 hover:text-white-900 hover:ease-in-out focus:text-emerald-900 disabled:text-black/30 motion-reduce:transition-none lg:px-2 [&.active]:text-emerald-900'
                        data-te-nav-link-ref
                      >
                        Sigma Files
                      </a>
              ...
              <div className='absolute mt-5 lg:right-2'>
                <div className='p-3 relative inline-flex'>
                  <div className='relative pb-3  flex flex-wrap items-stretch'>
                    <div className='mr-2'>
                      <button
                        className={
                          'text-black hover:bg-[#6941c6] hover:text-[#fff] capitalize rounded-lg px-[14px] py-[6px] bg-[#fff] text-center flex'
                        }
                        aria-label='menu'
                        onClick={handleClick}
                      ...
                      >
                        <span className='pl-2 pr-2'>
                          <svg
                            ...
                            height='20'
                            viewBox='0 0 20 20'
                            fill={filterValueOfTitle || filterValueOfSource ? '#1D2939' : 'none'}
                          >
                            <path
                      ...
                      <MenuItem>
                        <button
                          className={`flex  w-[80px] ${
                            filterValueOfTitle || filterValueOfSource
                              ? '  cursor-pointer'
                              : 'cursor-not-allowed opacity-50'
                          }`}
                          disabled={filterValueOfTitle || filterValueOfSource ? false : true}
                          onClick={handleClickClear}
                        >
                          <li className='m-1 '>Clear All</li>
                        </button>
                      </MenuItem>
                      <MenuItem onClick={(e) => handleMenuOpen0(e, 'filter_by_title')}>
                        <button className='flex '>
                          <svg
                            className='mt-1'
                            xmlns='http://www.w3.org/2000/svg'
                            ...
                            height='20'
                            viewBox='0 0 20 20'
                            fill={filterValueOfTitle ? '#1D2939' : 'none'}
                          >
                            <path
                              d='M2.82174 4.72239C2.19146 4.01796 1.87631 3.66574 1.86443 3.3664C1.8541 3.10636 1.96585 2.85643 2.16652 2.69074C2.39753 2.5 2.87015 2.5 3.8154 2.5H16.1845C17.1298 2.5 17.6024 2.5 17.8334 2.69074C18.0341 2.85643 18.1458 3.10636 18.1355 3.3664C18.1236 3.66574 17.8084 4.01796 17.1782 4.72239L12.423 10.037C12.2973 10.1774 12.2345 10.2477 12.1897 10.3276C12.15 10.3984 12.1208 10.4747 12.1032 10.554C12.0833 10.6435 12.0833 10.7377 12.0833 10.9261V15.382C12.0833 15.5449 12.0833 15.6264 12.057 15.6969C12.0338 15.7591 11.996 15.8149 11.9468 15.8596C11.8912 15.9102 11.8155 15.9404 11.6642 16.001L8.83088 17.1343C8.52459 17.2568 8.37145 17.3181 8.24851 17.2925C8.14101 17.2702 8.04666 17.2063 7.98599 17.1148C7.91662 17.0101 7.91662 16.8452 7.91662 16.5153V10.9261C7.91662 10.7377 7.91662 10.6435 7.89672 10.554C7.87907 10.4747 7.84992 10.3984 7.81019 10.3276C7.7654 10.2477 7.70258 10.1774 7.57694 10.037L2.82174 4.72239Z'
                              stroke='#475467'
                          ...
                          <li className='m-1 '>Filter by Title</li>
                        </button>
                      </MenuItem>
                      <MenuItem onClick={(e) => handleMenuOpen0(e, 'filter_by_source')}>
                        <button className='flex '>
                          <svg
                            className='mt-1'
                            xmlns='http://www.w3.org/2000/svg'
                            ...
                            height='20'
                            viewBox='0 0 20 20'
                            fill={filterValueOfSource ? '#1D2939' : 'none'}
                          >
                            <path
                              d='M2.82174 4.72239C2.19146 4.01796 1.87631 3.66574 1.86443 3.3664C1.8541 3.10636 1.96585 2.85643 2.16652 2.69074C2.39753 2.5 2.87015 2.5 3.8154 2.5H16.1845C17.1298 2.5 17.6024 2.5 17.8334 2.69074C18.0341 2.85643 18.1458 3.10636 18.1355 3.3664C18.1236 3.66574 17.8084 4.01796 17.1782 4.72239L12.423 10.037C12.2973 10.1774 12.2345 10.2477 12.1897 10.3276C12.15 10.3984 12.1208 10.4747 12.1032 10.554C12.0833 10.6435 12.0833 10.7377 12.0833 10.9261V15.382C12.0833 15.5449 12.0833 15.6264 12.057 15.6969C12.0338 15.7591 11.996 15.8149 11.9468 15.8596C11.8912 15.9102 11.8155 15.9404 11.6642 16.001L8.83088 17.1343C8.52459 17.2568 8.37145 17.3181 8.24851 17.2925C8.14101 17.2702 8.04666 17.2063 7.98599 17.1148C7.91662 17.0101 7.91662 16.8452 7.91662 16.5153V10.9261C7.91662 10.7377 7.91662 10.6435 7.89672 10.554C7.87907 10.4747 7.84992 10.3984 7.81019 10.3276C7.7654 10.2477 7.70258 10.1774 7.57694 10.037L2.82174 4.72239Z'
                              stroke='#475467'
                          ...
                          <li className='m-1 '>Filter by Source</li>
                        </button>
                      </MenuItem>
                    </Menu>
                    {ctiReportFiles.length ? (
                      <Menu
                        className='mr-6'
                        style={{ maxHeight: 400 }}
                        anchorOrigin={{
                          vertical: 'top', // or 'bottom'
                          horizontal: 'left', // or 'left'
                        }}
                        transformOrigin={{
                          vertical: 'top', // or 'bottom'
                        ...
                        onClose={handleMenuClose0}
                      >
                        {filterTitle == 'filter_by_title' ? (
                          <>
                            {Array.isArray(ctiReportFiles) &&
                              ctiReportFiles.length > 0 &&
                              ctiReportFiles
                                .filter(
                                  (item: any) => !item.errorDesc && item?.status === 'COMPLETE',
                                )
                          ...
                          <>
                            {arraysource.map((item: any, key: any) => (
                              <MenuItem
                                key={key}
                                value={item.title}
                                onClick={(event) => handleClickMenusource(event, item)}
                              >
                                <ul>
                                  <li className='m-1'>
                                    <div className='flex justify-between items-center gap-[4px]'>
                                      {filterValueOfSource?.title == item.title && (
                                        <>
                                          <span>
                                            <svg
                                              xmlns='http://www.w3.org/2000/svg'
                                      ...
                                      <span
                                        className={`${
                                          filterValueOfSource?.title !== item.title && 'pl-[12px]'
                                        }`}
                                      >
                                        {item.title}
                                      </span>
                    ...
                    <button
                      ...
                      className={`${
                        selectedRows.length == 0
                          ? 'cursor-not-allowed opacity-50'
                          : ' hover:bg-[#6941C6] cursor-pointer'
                      }
                     text-white  capitalize rounded-lg px-[14px] py-[7px] bg-[#EE7103] text-center flex`}
                      aria-label='menu'
                    ...
                    <Menu anchorEl={anchorEls} open={Boolean(anchorEls)} onClose={handleMenuClose}>
                      <MenuItem sx={{ width: '12vw' }} onClick={handleDownload}>
                        <span className='mr-5'>
                          <svg
                            xmlns='http://www.w3.org/2000/svg'
                            width='20'
                      ...
                      <MenuItem
                        onClick={(event: any) => {
                          shareFile(event, selectedRows, 'copyTo')
                        }}
                        disabled={selectedRows?.length <= 10 ? false : true}
                      >
                        <span className='mr-5'>
                          <svg
                            xmlns='http://www.w3.org/2000/svg'
                        ...
                        <span>Copy Sigma File</span>
                      </MenuItem>
                      {deleteall.length > 0 && (
                        <MenuItem onClick={handleDeleteMultipleFiles}>
                          <span className='mr-5'>
                            <svg
                              xmlns='http://www.w3.org/2000/svg'
                              fill='none'
                  ...
                  <div className='ml-2 '>
                    <div className='relative inline-flex lg:top-0 lg:pt-0 '>
                      <div className='relative flex flex-wrap items-stretch '>
                        <input
                          type='search'
                          className='relative m-0 block w-[19rem] flex-auto rounded-l-lg p-1.5 border-t border-l border-b border-solid border-neutral-300 bg-white text-base font-normal leading-[1.6] text-neutral-700'
                          placeholder='Search...'
                          aria-label='Search'
            ...
            <Grid item xs={4} container justifyContent='center' alignItems='center'>
              {selectedRows.length > 0 ? (
                <p className='text-white font-medium lg:pr-2'>
                  {selectedRows.length + '    ' + 'Sigma File(s) Selected'}
                </p>
              ) : (
                ''
              )}
      ...
      <>
        <div onClick={VaultDropdown}>
          ...
          >
            <DataGrid
              sx={{
                ...
                color: 'white',
                border: 'none',
                height: { md: height - 150, lg: height - 250, xl: height - 250 },
                '&>.MuiDataGrid-main': {
            ...
            <>
              <div className='justify-center backdrop-blur-sm items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none p-35'>
                <div className='md:text-sm 2xl:text-lg relative w-auto my-6 mx-auto w-2/5'>
                  <div className='md:text-sm 2xl:text-lg border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none text-[#000]'>
                    <div className='items-start justify-between p-5 border-solid border-slate-200 rounded-t'>
                      ...
                      <p className='md:text-sm 2xl:text-lg justify-center text-sm items-center text-center mt-2'>
                        Are you sure you want to delete this Sigma File(s)?
                      </p>
                      <p className='md:text-sm 2xl:text-lg justify-center text-sm items-center text-center mt-2'>
                    ...
                    <div className='flex p-2 border-solid border-slate-200 rounded-b gap-4'>
                      <button
                        type='button'
                        className='md:text-sm 2xl:text-lg  w-full bg-white-900 text-sm font-medium text-gray-900 border rounded-lg justify-center font-bold px-3 py-2 text-xs inline-flex '
                  ...
                  <div
                    className={` ${
                      mode === 'translate' || mode == 'view'
                        ? ' border-0 rounded-lg h-full border border-pink-200 shadow-lg relative flex flex-col bg-[#1D2939] outline-none focus:outline-none'
                        : ' border-0 rounded-lg h-full  border shadow-lg relative flex flex-col bg-[#1D2939] outline-none focus:outline-none'
                    }`}
                  >
                      ...
                      >
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              if (filterTitle == 'filter_by_title' && filterState) {
                                handleClickMenu(e, filterValueOfTitle)
                              } else if (filterTitle == 'filter_by_source' && filterValueOfSource) {
                                handleClickMenusource(e, filterValueOfSource)
                              } else {
                                fetchdetails()
                              }
                              setSampleKPMG(false)
          ...
          {inspectView && (
            <InspectSectionView
              setInspectView={setInspectView}
              ctiSectionId={ctiSectionId}
...
export default VaultPermission
