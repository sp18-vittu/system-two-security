...
const convertToFlowElements = (
  ...
  const getNodePosition = (
    ...
    const spacing = adjustedVerticalSpacing / (nodesAtLevel + 1)
    let Y

    if (level < 2) {
      if (childNumber % 2 !== 0) {
        Y = adjustedVerticalSpacing / 2 - (Math.floor(childNumber / 2) + 1) * spacing
      } else {
        Y = adjustedVerticalSpacing / 2 + Math.floor(childNumber / 2) * spacing
      }
    } else {
      Y = parentY !== undefined ? parentY + (childNumber + 1) * spacing : 0
    }
...
const HorizontalFlow = ({ messages, planDetails }: any) => {
  ...
  useEffect(() => {
    const initialTreeData: any = messages?.data?.root
